...
class OnlineTest extends JFrame implements ActionListener {  
    ...
    int m[] = new int[10];  
    boolean isCorrect[] = new boolean[10];  // To store correctness of answers
    ...
    public void actionPerformed(ActionEvent e) {  
        if (e.getSource() == b1) {  
            if (check()) {  
                count++;  
                isCorrect[current] = true;  // Mark the current question as correct
            } else {  
                isCorrect[current] = false; // Mark the current question as incorrect
            }  
            current++;  
            set();    
            if (current == 9) {  
        ...
        if (e.getActionCommand().equals("Result")) {  
            ...
            result.append("Correct Answers: ").append(count).append("\n\n");  
            for (int i = 0; i < 10; i++) {  
